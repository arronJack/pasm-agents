...
> **本仓（pasm-agents）是成品智能体集，分两层**：
>
> | 层 | 是什么 | 章节 |
> |---|---|---|
> | **产品智能体**（3 个） | 面向使用者：游戏 NPC / 老人陪伴 / 学习陪伴 | §八 |
> | **验证智能体**（7 个） | 面向开发者：**可反复运行、产出结构化结论的长期验证器** | §一~§七 |
>
> 验证智能体零依赖、不常驻、不需要服务器 —— 长期验证要的是可重复、可比较、可归档。
>
> **写自己的智能体**请去基座仓 `pasm-skills`：`docs/BUILD-AGENT.md`（手把手）+ `templates/`（脚手架）。
> 本文件讲的是**本仓这 10 个智能体怎么设计、为什么不那么设计**（含选型分析与真问题表）。
...
> 完整教程在基座仓：`pasm-skills/docs/BUILD-AGENT.md`。
> 下面是最小骨架。
...
三者共享 `BaseAgent`（定义在**基座仓**的 `pasm_skills/sdk/base.py`），差异只在 persona / 动作池 / 聊天模板。
