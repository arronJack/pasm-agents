"""独立跑 product-verifier —— 产品层（npc / companion / tutor）公开 API、隔离性、真实行为与反例

    python agents/verifiers/product_verifier/run.py

等价于在仓库根执行（这里只是帮你把发现路径配好）：

    python -m pasm_skills run product-verifier
"""
import os
import subprocess
import sys
...
raise SystemExit(subprocess.call(
    [sys.executable, "-m", "pasm_skills", "run", "product-verifier"], env=env,
))
