...
**第 4 个产品智能体 `CustomerServiceAgent`（智能客服）；顺带修掉一个会跨租户串资料的落点缺陷，并补上两处守门登记。**

### 增

- **`CustomerServiceAgent` —— 智能客服（第 4 个产品）**
  - `from pasm_agents import CustomerServiceAgent`；CLI：`pasm-agents demo|run|selftest customer-service`。
  - **就资料作答 + 绝不编造**：只认 `knowledge_facts()`（带 `source` 的知识），
    查不到就如实说不知道并给出客服热线 —— 不硬凑一个答案。
  - **相关性闸门**（本版最值得说的一处）：命中必须落在条目的**标题或标签**上，
    **或**分数达到 `score_floor`（默认 4.0）。
    起因是实测到的一个真问题：问「请问 CEO 的私人邮箱是多少」，
    资料库靠正文里一个「邮箱」命中了**发票开具**条目，于是**答非所问**。
    实测分离度：真命中 **6.00 ~ 24.50**，假命中 **2.29**。
    之所以主判据**不用**绝对分数：分数带时间衰减（`0.6 + 0.4 × recency`），
    资料库放一个月后真命中被压到 0.6 倍，任何固定阈值都会开始"失忆"；
    而"命中落在标题/标签上"与分数、与时间都无关。
  - **客诉转人工**：四类（投诉/曝光、法律/监管、安全/伤害、要求赔偿）自动识别，
    按 `salience=5` 写入**不可被日常闲聊挤掉**的记忆，并返回升级上下文
    （原因 / 会话 / 用户 / 热线 / 累计次数）交给你的通知渠道。
    词表刻意**只收明确指向该场景的短语**：「退款」是正常问询，「**要求**退款」才是客诉；
    安全类只收**结果性**表达（「过敏了」「受伤了」），不收光杆「过敏」——
    因为「这个成分会让我过敏吗」是提问，升级它等于乱升级。判不出来就放行。
  - **建在框架的 `BaseApplication` 上**（而非另三个产品的基座 `BaseAgent`）：
    "客服"的核心能力是**资料库**，而资料库是框架的 `knowledge_base` 插件。
    好处是**轻量档下资料库功能完整**，不像另几个产品在 `tier=light` 时丢能力。
  - **可选接大模型**：`llm={...}` 是普通配置字典，不绑定任何厂商；
    LLM 不可用时**自动回落到资料库作答**，永远有回应而不是抛错。
  - **可机读快照** `snapshot()`：资料库规模 / 情绪 / 交互数 / 客诉次数 / 档位；
    资料库未启用时 `kb` 返回 `None`（**不假装有**）。
  - 自检护栏 `python -m pasm_agents.selftest_customer_service`（**75 项**）：
    含「问 CEO 邮箱不许答成发票政策」「普通抱怨不许升级」「A 店资料不许被 B 店检索到」
    等反例。**反向验证已验**：注入 4 种缺陷（去掉相关性闸门 / 放宽客诉词表 /
    取消资料库隔离 / 让查不到时编造）全部被抓红，还原后复绿。

### 修

- **资料库默认落点是全机共享的，会让同机多租户互相串资料**
  - 框架 `knowledge_base` 插件的默认落点是 `~/.pasm_framework/kb` ——
    **全机共享**。两个客服（两家店 / 两个租户）跑在同一台机器上时，
    A 的 FAQ 会被检索进 B 的答复里。
  - 这属于**跨租户数据泄漏**，而且症状极难排查（"我喂的资料为什么答不上来 /
    为什么冒出别人的资料"）。
  - 现在产品层默认落在 `<persist_dir>/kb`，各智能体一个库。自测也一并受益：
    传了 `persist_dir` 就完全自洽，不会读到开发机上的真实资料库。
- **`tools/check_skill_docs.py` 的 `KNOWN_REPOS` 白名单过期，反过来误报合法引用**
  - `pasm-framework`（2026-09-20 拆出）与 `pasm-customer-service`（2026-09-21 新建）
    落地后都没登记，于是 4 份技能正文里**正确**的 `pip install pasm-framework` /
    `git clone .../pasm-framework.git` 被判为"未知仓库/未知包"。
  - 危险的不是"检查变松"而是**检查开始误报正确的东西** ——
    一旦人习惯性无视它，它守的那条真规则（"别让用户去基座仓找智能体"）也就失效了。
  - 已补齐白名单，并把"新仓落地必须同步登记"写进注释。

### 增（守门登记）

- **`surface-guard` 覆盖第 4 个产品与第 5 个技能**：已知产品清单加 `CustomerServiceAgent`，
  技能清单加 `cs-agent`。产品层是**数据驱动**的（扫公开 API 里所有 `BaseAgent` 子类），
  所以客服**自动**被覆盖到 —— 因为 `BaseApplication` 本身就是 `BaseAgent` 的子类。
  > ⚠️ 反直觉点，写进注释了：真正会让它**漏检**的不是"换了底座"，
  > 而是**忘了在 `pasm_agents/__init__.py` 里按需导出它**。
- **`product_verifier` 加第 4 产品的产品层断言**：公开 API、资料库隔离、
  "就资料作答 / 查不到如实说"、客诉识别边界，含反例。
- **新技能包 `pasm-cs-agent`**（正文 `skill/SKILL.cs-agent.body.md`），
  并说明技能名与 PyPI 包名 `pasm-customer-service` 的分工：
  前者是"放进平台的智能体"，后者是"含 DB→KB 同步 / MCP 服务 / Web 壳的整套系统"。
...
- **`regression` 的 Lite 探针从没走到真正的注册路径**
  - 旧代码注释断言「引擎在 `import pasm_lite` 时注册」，于是只 `import pasm_lite`；
    但 PASM-Lite 的引擎实际是在 **`engine.py` 末尾**自注册的
    （`REGISTRY.register(ENGINE_NAME, ...)`），而 `pasm_lite.py` 只导入 `learning`/torch。
  - 后果：这一行**永远只能报「档位不可比 / 引擎列表为空」**，无论 Lite 是否真的可用 ——
    凡是能永远给出同一个结果的检查，都不是检查。
  - 修法：补 `import engine`，让探针走**真实**的注册路径。

### 影响

- 刷新后 `baselines/core.json` 才第一次记录到 Lite 的**真实**引擎：
  `lite.engines` 由「回显核心列表」变为 `["pasm-lite"]`；`core.engines` 保持
  `["pasm", "pasm-light"]` 不变 → **没有能力消失，是尺子修好了**。
- 已用改动前的核心（`git archive HEAD`）跑同一套件做反例对照：两边 WARN 与数值
  完全一致 → 本次修复**未引入任何行为回归**。

### 说明（解释器档位）

- 基线里的 `python` / `torch` 字段是**可比性指纹**。本次刷新记录的解释器与上一版不同
  （含 torch 的解释器已从 `3.13.14` 变为本机 `3.12.6`）。
  换解释器导致的 WARN 属**环境差异**，已在报告里降级说明，**不当作能力退化**。
