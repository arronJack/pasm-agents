...
name: pasm-npc
description: >-
  PASM game NPC agent - drop an NPC into your game that actually remembers the player, has emotion,
  and gets shaped by player feedback over time. Use when NPCs must recall what the player did, vary a
  line by mood, or shift behavioural preference through long-term praise/scolding, or when you want to
  bolt a cognitive core onto an existing game. Zero LLM dependency, offline-runnable, state persists
  to ~/.pasm-agents/<id>/. Keywords: pasm, npc, game agent, memory, emotion, persona, salience,
  offline.
version: 0.5.0
license: MIT
author: arronzheng
...
6. **触发词要"专指"，别用宽词**：
   - 问身份只认「你叫什么 / 你是谁 / 你的名字 / 怎么称呼」这类**明确问句**，
     **不放光杆「名字」** —— 否则「我名字叫张三 / 这花有名字吗 / 我给你起个名字吧」
     全会被当成在问 NPC 是谁（2026-09-15 探针实测 4/4 误触发）；
   - **自称由 persona 决定**（`self_ref`，留空用中性「我」），
     别在模板里写死「老朽」—— 否则「卖花的小姑娘」也会说"老朽还记着呢"。

   回归护栏：`python -m pasm_agents.selftest_npc`（12 项，秒级，退出码 0/1）。
   **改触发词或回复模板就顺手跑它** —— 这条通路坏了，别的测试不会报错。
...
`self_ref`（可选）是 **NPC 的自称**：留空用中性「我」，老派角色可写「老夫」「老朽」。
它**必须由 persona 给** —— 别指望模板默认值，写死就会出现
「卖花的小姑娘说：老朽还记着呢」这种**人设泄漏**。
